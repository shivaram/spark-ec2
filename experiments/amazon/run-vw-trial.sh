...
time KEYSTONE_MEM=97g /root/keystone/bin/run-pipeline.sh pipelines.speech.VWTimitEval --dataLocation /$TIMESTAMP/test/data --vwLocation /root/vowpal_wabbit/vowpalwabbit/vw --modelLocation /root/keystone/$TIMESTAMP/model

popd > /dev/null
echo "wrote to hdfs /$TIMESTAMP/out"
root@ip-10-43-139-80 timit]$ cat run-vw-trial.sh 
#!/bin/bash
set -e
